...
public class BoardDAO {
	...
	public ArrayList<PostVO> getPostingList(PagingBean pagingBean) throws SQLException{
		...
		try{
			...
			sql.append("FROM(SELECT row_number() over(ORDER BY NO DESC) as rnum,no,title,hits,to_char(time_posted,'YYYY.MM.DD') as time_posted,id FROM board_paging ");
			sql.append(")B, BOARD_MEMBER_paging M WHERE B.id=M.id AND rnum BETWEEN ? AND ? ");	
	...
	public int getTotalPostCount() throws SQLException {
		...
		try {
		con=getConnection();
		String sql="select count(*) from board_paging";
		pstmt=con.prepareStatement(sql);
		rs=pstmt.executeQuery();
		if(rs.next())
